"""Object-oriented business logic for the Administrator role."""
...
class Administrator(User):
    """Administrator: encapsulates business logic according to the Class Diagram; independent of Flask."""
    ...
    def list_users(self):
        ...
        try:
            users = db.fetch_all("""
                select u.user_id, u.username, u.full_name as fullname, u.email, u.role,
                       u.active_status as status, s.student_id, m.major_code, l.lecturer_id,
                       coalesce(array_agg(distinct c.course_code) filter (where c.course_code is not null), '{}') as qualifications
                from users u
                ...
                group by u.user_id,s.student_id,m.major_code,l.lecturer_id
        ...
        """,
            ...
            """,
                ...
                for course_code in dict.fromkeys(
                    ...
                    (qualification_id, lecturer_id, course_id)
            ...
            if role == "student":
                ...
                (student_id, user_id, date_of_birth, major_id)
                values (%s, %s, %s, (select major_id from majors where major_code=%s))
            """,
                    (user_id, user_id, dob, major_code),
                )
            db.conn.commit()
            return (
                {
        ...
        try:
            ...
            if major_code is not None:
                db.execute(
                    """
                    update students
                    set major_id=(select major_id from majors where major_code=%s)
                    where user_id=%s
                    """,
                    (major_code, user_id),
                )
    ...
    def manageCourse(self):
        ...
        try:
            ...
            from courses c
            left join courses p
                on p.course_id=c.prerequisite_course_id
    ...
    def create_course(self, data):
        ...
        db = get_db()
        try:
            if prerequisite and not db.fetch_one(
                "select course_id from courses where course_code=%s", (prerequisite,)
            ):
                return ({"error": "Prerequisite course does not exist"}, 400)
            db.execute_query(
                """
            insert into courses
            (
            ...
            values (%s, %s, %s, %s, (select course_id from courses where course_code=%s), %s)
        """,
                (
                    course_code,
                    course_name,
    ...
    def update_course(self, course_code, data):
        ...
        try:
            ...
            current = db.fetch_one(
                "select course_name, credit, max_capacity from courses where course_code=%s",
                (course_code,),
            )
            if not current:
            ...
            db.execute_query(
                """
                update courses
                set course_name=%s, credit=%s,
                    prerequisite_course_id=(select course_id from courses where course_code=%s),
                    available_seats=%s + (available_seats - max_capacity), max_capacity=%s
                where course_code=%s
                """,
                (course_name, credit, prerequisite, max_capacity, max_capacity, course_code),
            )
    ...
    def manageCurriculum(self, major_code):
        ...
        try:
            ...
            select
                cu.curriculum_id,
                (select ref.major_code from majors ref where ref.major_id=cu.major_id) as major_code,
                c.course_code,
                c.course_name,
                c.credit,
                (select ref.course_code from courses ref where ref.course_id=c.prerequisite_course_id) as prerequisite_course_code,
            ...
            where (select ref.major_code from majors ref where ref.major_id=cu.major_id) = %s
            order by cu.recommended_semester, c.course_code
        """,
                (major_code,),
            )
            return curriculum
    ...
    def add_curriculum(self, data):
        ...
        """,
                (curriculum_id, major_code, course_code, semester),
            )
    ...
    def generateRegistrationDemandReport(self, query):
        ...
        try:
            ...
            select
                ...
                filter (
                    where lower(r.status::text) = 'registered'
            ...
            left join registrations r
               ...
               and r.period_id = %s
            where (%s is null or (select ref.major_code from majors ref where ref.major_id=cu.major_id) = %s)
    ...
    def demand_students(self, course_code, query):
        ...
        try:
            ...
            select
                ...
                u.email,
                lower(r.status::text) as registration_status
            ...
            where r.period_id = %s
              and (select ref.course_code from courses ref where ref.course_id=r.course_id) = %s
              and lower(r.status::text) = 'registered'
    ...
    def list_assignments(self, query):
        ...
        try:
            ...
            select
                ...
                ta.semester_id,
                ta.lecturer_id,
                (select ref.course_code from courses ref where ref.course_id=ta.course_id) as course_code,
    ...
    def qualified_lecturers(self, course_code):
        ...
        try:
            ...
            join users u
                on u.user_id = l.user_id
            where (select ref.course_code from courses ref where ref.course_id=q.course_id) = %s
    ...
    def add_qualification(self, data):
        ...
        try:
            ...
            values (%s, %s, (select course_id from courses where course_code=%s))
        """,
                (qualification_id, lecturer_id, course_code),
            )
    ...
    def remove_qualification(self, lecturer_id, course_code):
        ...
        try:
            ...
            where lecturer_id=%s and course_id=(select course_id from courses where course_code=%s)
        """,
            ...
            db.execute_query(
                'delete from lecturer_qualifications where lecturer_id=%s and course_id=(select course_id from courses where course_code=%s)',
                (lecturer_id, course_code),
            )
    ...
    def manageStudent(self):
        """Manage student accounts through administrator CRUD operations."""
        students = []
    ...
    def manageLecturer(self):
        """Manage lecturer accounts through administrator CRUD operations."""
        lecturers = []
        for user in self.list_users():
