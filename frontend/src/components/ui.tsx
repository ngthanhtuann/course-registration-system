...
const BTN_VARIANT: Record<BtnVariant, string> = {
  ...
  secondary:
    "bg-slate-100 text-slate-700 hover:bg-slate-200 active:bg-slate-300 focus:ring-2 focus:ring-slate-300",
  danger:
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      />
      <div
        className={`relative bg-white w-full ${sizeMap[size]} rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90vh]`}
      >
...
export function StatCard({
  ...
  return (
    <Card className="p-5 flex items-center gap-4">
      <div
        className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${colors[color] ?? colors.blue}`}
      >
        {icon}
      </div>
