"""Kiểm tra AST backend, không khởi chạy Flask, không import ứng dụng hay kết nối DB.

Chạy từ bất kỳ thư mục nào: python backend/checks/static_check.py
Ruff bổ sung việc kiểm tra biến chưa định nghĩa và các lỗi Python phổ biến.
"""
...
def module_name(path):
    """Đổi đường dẫn Python thành tên module để phân tích import nội bộ."""
...
def check_backend():
    """Kiểm tra cú pháp, chú thích, lớp UML, hợp đồng route, lời gọi và SQL tham số."""
    errors = []
    trees = {}
    definitions = 0
    sql_count = 0
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if not ast.get_docstring(node):
                    errors.append(
                        f"{path.name}:{node.lineno}: thiếu giải thích cho {node.name}"
                    )
                ...
                if (
                    ...
                    if (
                        ...
                        if sql.value.count("%s") != len(params.elts):
                            errors.append(
                                f"{path.name}:{node.lineno}: số tham số SQL không khớp"
                            )
                ...
                if target in trees:
                    ...
                    for alias in node.names:
                        if alias.name != "*" and alias.name not in exports:
                            errors.append(
                                f"{name}: import {alias.name} không tồn tại trong {target}"
                            )
                elif node.level:
                    errors.append(f"{name}: module tương đối không tồn tại: {target}")
                if name.startswith("domain.") and target in (
                    "flask",
                    "http_support",
                    "security",
                ):
                    errors.append(f"{name}: nghiệp vụ đang phụ thuộc lớp HTTP {target}")

    visited = set()

    def visit(name, stack):
        """Duyệt đồ thị import nội bộ để phát hiện phụ thuộc vòng."""
        if name in stack:
            errors.append("Import vòng: " + " -> ".join(stack + [name]))
            return
        if name in visited:
            return
    ...
    for entry in contract:
        funcs = {
            n.name: n
            for n in trees[entry["module"]].body
            if isinstance(n, ast.FunctionDef)
        }
        func = funcs.get(entry["name"])
        if func is None:
            errors.append(f"Mất API {entry['module']}.{entry['name']}")
            continue
        if [ast.unparse(d) for d in func.decorator_list] != entry[
            "decorators"
        ] or ast.unparse(func.args) != entry["args"]:
            errors.append(
                f"Thay đổi route/quyền/tham số: {entry['module']}.{entry['name']}"
            )

    routed_calls = 0
    for module in ("auth", "admin", "student", "lecturer"):
        for func in trees[module].body:
            if not isinstance(func, ast.FunctionDef):
                continue

            # Ghi nhận ví dụ: student = Student(current_user).
            objects = {}
            ...
            for node in ast.walk(func):
                ...
                if method is None:
                    errors.append(
                        f"{module}: phương thức không tồn tại {class_name}.{node.func.attr}"
                    )
                    continue
                ...
                if (
                    ...
                    errors.append(f"{module}: sai tham số cho {method.name}")

            if calls_in_route == 0:
                errors.append(
                    f"{module}.{func.name}: không tìm thấy lời gọi đối tượng nghiệp vụ"
                )
