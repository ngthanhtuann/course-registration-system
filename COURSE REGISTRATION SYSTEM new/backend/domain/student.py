"""Nghiệp vụ hướng đối tượng cho vai trò Student."""
...
class Student(User):
    """Student: đóng gói nghiệp vụ theo Class Diagram; không phụ thuộc Flask."""
    ...
    def viewProfile(self):
        ...
        try:
            row = db.fetch_one(
                """select s.student_id, s.dob, s.major_code, u.user_id, u.username, u.fullname, u.email, u.role
    ...
    def registerCourse(self, data):
        ...
        # Bước 1: Lấy mã học phần và đợt đăng ký từ dữ liệu frontend gửi lên.
        student = self.identity
        data = data or {}
        ...
        db = get_db()
        try:
            # Bước 2: Khóa học phần để các yêu cầu đăng ký không lấy cùng chỗ cuối.
            course = db.fetch_one(
            ...
            # Bước 3: Kiểm tra đợt đăng ký có đang mở hay không.
            period = db.fetch_one(
            ...
            if not student_record:
                db.conn.rollback()
                return ({"error": "student profile not found"}, 404)
            # Bước 4: Học phần phải thuộc chương trình đào tạo của sinh viên.
            ...
            # Bước 5: Không đăng ký trùng hoặc đăng ký lại môn đã đạt.
            ...
            # Bước 6: Nhờ đối tượng Course kiểm tra môn tiên quyết và chỗ trống.
            ...
            # Bước 7: Tạo đối tượng đăng ký, lưu rồi xác nhận toàn bộ giao dịch.
            ...
            # Nếu có lỗi, hủy thay đổi chưa lưu để không tạo đăng ký dở dang.
            db.conn.rollback()
