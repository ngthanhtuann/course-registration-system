"""Nghiệp vụ hướng đối tượng cho vai trò Administrator."""
...
class Administrator(User):
    """Administrator: đóng gói nghiệp vụ theo Class Diagram; không phụ thuộc Flask."""
    ...
    def list_users(self):
        ...
        try:
            ...
            if "fullname" in column_names:
                users = db.fetch_all("""
                select u.user_id, u.username, u.fullname, u.email, lower(u.role::text) as role,
                       u.active_status as status, s.student_id, s.major_code, l.lecturer_id,
            ...
            """)
                ...
                select u.user_id, u.username,
                       ...
                       s.student_id, m.major_code, l.lecturer_id,
                ...
                group by u.user_id,u.username,u.role,u.active_status,s.student_id,m.major_code,
                         s.full_name,s.email,l.lecturer_id,l.full_name,l.email
                order by u.role, u.user_id
            """)
            return users
        finally:
    ...
    def create_user(self, data):
        ...
        """,
            ...
            db.execute(
                """
            insert into users
            (user_id, username, password, password_hash, fullname, email, role)
        ...
        """,
            ...
            if role == "lecturer":
                ...
                (lecturer_id, user_id, full_name, email, fullname)
                values (%s, %s, %s, %s, %s)
            """,
                    (user_id, user_id, fullname, email, fullname),
                )
            ...
            if role == "student":
                ...
                (student_id, user_id, full_name, date_of_birth, dob, email, major_id, major_code)
                values (%s, %s, %s, %s, %s, %s,
                        (select major_id from majors where major_code=%s), %s)
            """,
                    (user_id, user_id, fullname, dob, dob, email, major_code, major_code),
                )
            db.conn.commit()
            return (
                {
    ...
    def update_user(self, user_id, data):
        ...
        try:
            ...
            db.execute(
                """update users set fullname=coalesce(%s,fullname), email=coalesce(%s,email),
            ...
            db.execute(
                "update students set full_name=coalesce(%s,full_name), email=coalesce(%s,email) where user_id=%s",
                (fullname, email, user_id),
            )
            db.execute(
                "update lecturers set full_name=coalesce(%s,full_name), fullname=coalesce(%s,fullname), email=coalesce(%s,email) where user_id=%s",
                (fullname, fullname, email, user_id),
            )
            if major_code is not None:
                db.execute(
                    """
        ...
        try:
            ...
            db.execute_query(
                ...
                set course_name=%s, credit=%s, prerequisite_course_code=%s,
                    max_capacity=%s, capacity=%s,
                    available_seats=%s + (available_seats - capacity)
                where course_code=%s
                """,
                (course_name, credit, prerequisite, max_capacity, max_capacity, max_capacity, course_code),
            )
    ...
    def create_period(self, data):
        ...
        """,
                (period_id, semester_id, period_name, period_name, start_date, end_date, start_date, end_date),
            )
    ...
    def update_period(self, period_id, data):
        ...
        """,
            ...
            where period_id = %s
        """,
                (semester_id, period_name, period_name, start_date, end_date, start_date, end_date, period_id),
            )
    ...
    def manageStudent(self):
        """Quản lý tài khoản sinh viên qua các thao tác CRUD của quản trị viên."""
        students = []
    ...
    def manageLecturer(self):
        """Quản lý tài khoản giảng viên qua các thao tác CRUD của quản trị viên."""
        lecturers = []
        for user in self.list_users():
